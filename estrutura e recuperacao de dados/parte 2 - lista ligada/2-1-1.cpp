...
void insere_dobro (no ** pri, elemento a){
    ...
    //no novo nó vamos inserir a (no caso 1) e deixar seu link inicialmente como NULL
    novo->info = a;
    novo->link = NULL;

    //definimos o comeco da lista, entregando novo a pri e também apontando p para pri.
    *pri = novo;
    p = *pri;

    //enquanto o elemento A for menor que 1000...
    while (a < 1000){
        //dobramos o valor de a
        a = a * 2;

        //verificamos se a ainda é menor que 1000
        if (a < 1000){
            //criamos um novo nó
            novo = novo_no();

            //atribuimos seu link como NULL e inserimo o elemento a em seu info
            novo->link = NULL;
            novo->info = a;

            //ligamos a nossa lista ao novo nó, através do link de p
            p->link = novo;

            //"caminhamos" com p para a nova posicao criada
            p = p->link;
        }
